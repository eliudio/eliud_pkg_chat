...
class ChatListWidget extends StatefulWidget with HasFab {
  ...
  ChatWidgetProvider? widgetProvider;
  bool? readOnly;
  String? form;
  ChatListWidgetState? state;
  bool? isEmbedded;

  ChatListWidget({ Key? key, this.readOnly, this.form, this.widgetProvider, this.isEmbedded, this.listBackground }): super(key: key);

  @override
  ChatListWidgetState createState() {
    state ??= ChatListWidgetState();
    return state!;
  }
...
//            app: accessState.app,
  ...
  Widget? getForm(value, action) {
    if (widget.form == null) {
      return ChatForm(value: value, formAction: action);
    } else {
      return null;
    }
  }
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      ...
      child: ListTile(
        ...
        subtitle: value.saying != null ? Center(child: StyleRegistry.registry().styleWithContext(context).frontEndStyle().textStyle().text(context, value.saying!)) : Container(),
      ),
    );
  }
}
