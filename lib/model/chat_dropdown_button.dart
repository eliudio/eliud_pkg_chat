...
typedef ChatChanged(String? value, int? privilegeLevel,);

/* 
 * ChatDropdownButtonWidget is the drop down widget to allow to select an instance of Chat
 */
class ChatDropdownButtonWidget extends StatefulWidget {
  final AppModel app;
  final int? privilegeLevel;
  final String? value;
  final ChatChanged? trigger;
  final bool? optional;

  /* 
   * construct a ChatDropdownButtonWidget
   */
  ChatDropdownButtonWidget({ required this.app, this.privilegeLevel, this.value, this.trigger, this.optional, Key? key }): super(key: key);

  /* 
   * create state of ChatDropdownButtonWidget
   */
  @override
  State<StatefulWidget> createState() {
    return _ChatDropdownButtonWidgetState(value);
  }
}

class _ChatDropdownButtonWidgetState extends State<ChatDropdownButtonWidget> {
  ChatListBloc? bloc;
  String? value;

  _ChatDropdownButtonWidgetState(this.value);
...
List<Widget> _widgets(ChatModel value) {
var app = widget.app;
var widgets = <Widget>[];
        ...
        return ListView(
          ...
          Row(children: [((false) == true)
            ? Container(
                ...
                child: dropdownButton<String>(
                      ...
                      items: items,
                      value: value,
                      hint: text(widget.app, context, 'Select a chat'),
          ...
          if (value != null) 
            Align(alignment: Alignment.topRight, child: button(
              ...
              label: 'Update',
              onPressed: () {
                updateComponent(context, widget.app, 'chats', value, (newValue, _) {
                  setState(() {
                    value = value;
                  });
                });
